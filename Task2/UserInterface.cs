...
namespace Course.Task2
{
    ...
    {
        static public void AddProductsToStorage(Storage storage)
        {
            while (true)
            {
        ...
        static private Meat CreateMeat()
        {
            ...
            Category category = (Category)Enum.GetValues(typeof(Category)).GetValue(int.Parse(Console.ReadLine()));
            Console.Write("Введiть номер сорту(1 - баранина, 2 - телятина, 3 - курятина, 4 - свинина) продукту: ");
            Sort sort = (Sort)Enum.GetValues(typeof(Sort)).GetValue(int.Parse(Console.ReadLine()));
            try
            {
        ...
        static private DairyProducts CreateDairyProducts()
        {
            Console.Write("Введiть назву продукту: ");
            string name = Console.ReadLine();
            Console.Write("Введiть цiну продукту: ");
